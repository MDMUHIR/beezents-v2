...
import { Menu, X, ArrowRight, ChevronDown, PhoneCall, Shield } from 'lucide-react';
  ...
  const navItems = [
    ...
    },
    {
      ...
      subItems: [
        ...
        { label: 'SaaS Intelligent Support', href: '/solutions/saas-customer-support' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-7 text-sm font-medium" aria-label="Main Navigation">
          {navItems.map(item => {
            ...
            if (item.hasDropdown) {
              const isDropdownOpen = item.label === 'Services' ? servicesDropdown : solutionsDropdown;
              const setDropdown = item.label === 'Services' ? setServicesDropdown : setSolutionsDropdown;
        ...
        <div className="hidden sm:flex items-center gap-3">
          <Link
            href="/admin"
            className="text-[11px] font-mono text-[#94A3B8] hover:text-[#0282EB] px-2 py-1 rounded transition-colors flex items-center gap-1"
            title="Admin CMS & API Diagnostic Portal"
          >
            <Shield className="w-3 h-3 text-[#0282EB]" />
            <span className="hidden xl:inline">CMS</span>
            {auth.isAuthenticated && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />}
          </Link>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden border-b border-slate-200 bg-white px-5 pt-3 pb-6 space-y-3 shadow-xl">
          ...
          <div className="pt-3 border-t border-slate-100 flex flex-col gap-2">
            ...
            <Link
              href="/admin"
              className="w-full flex items-center justify-center gap-2 text-[#0282EB] text-xs font-medium py-2 hover:bg-slate-50 rounded-lg"
            >
